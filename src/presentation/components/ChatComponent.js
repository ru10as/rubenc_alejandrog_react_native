...
const ChatComponent = ({ route, navigation }) => {
    ...
    // Datos canónicos de la conversación (los pasa quien abre el chat),
    // no se infieren del usuario actual para no corromper el documento.
    const {
        chatId,
        compradorId,
        compradorNombre,
    ...
    // Altura real del header (drawer + barra de estado) para compensar el teclado
    ...
    // Mostrar en la cabecera el nombre de la otra persona
    useEffect(() => {
        if (nombreOtro) navigation.setOptions({ title: nombreOtro });
    }, [navigation, nombreOtro]);

    // Suscripción en tiempo real a los mensajes de la conversación
    useEffect(() => {
        const q = query(
    ...
    const enviarMensaje = async () => {
        ...
        try {
            // Crea/actualiza el documento de la conversación (se crea en el primer mensaje)
            await setDoc(
                doc(db, 'chats', chatId),
                {
                    ...
                    camisetaId: camiseta?.id ?? null,
                    camisetaNombre: camiseta?.nombre ?? null,
                    camisetaImagen: camiseta?.imagen ?? null,
...
const styles = StyleSheet.create({
    ...
    input: { flex: 1, maxHeight: 120, backgroundColor: '#fff' },
});
